 ...
 * to process the file.
 *
 * Amazon MWS limits calls to 1,000 total calls per hour per seller account. For best performance,
 * you should limit your calls to SubmitFeed to no more than three feeds per hour per seller
 * account, although you can successfully call SubmitFeed up to 30 times per hour. Feed size is
 * limited to 2,147,483,647 bytes (2^31 -1) per feed. If you have a large amount of data to
 * post, however, we recommend when possible that you submit feeds smaller than this limit;
 * submit feeds when you have 30,000 records/items or four hours have passed since your last
 * submittal, whichever comes first. This ensures optimal feed processing performance.
 *
 * The client must transmit a User-Agent header line so that we can diagnose problematic HTTP
 * client software. For more information about the User-Agent header line, see the topic,
 * User-Agent Header.
 *
 * The Content-MD5 HTTP header is required when calling SubmitFeed. It must be computed as per
 * section 14.15 of the HTTP/1.1 Specification(http://www.ietf.org/rfc/rfc2616.txt). For more
 * information, see the topic, Using the Content-MD5 Header with SubmitFeed.
 *
 * The actual format of the FeedContent in the HTTP body of the SubmitFeed call varies by
 * marketplace, seller, product category, and by other factors.
 *
